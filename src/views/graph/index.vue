  ...
  <div class="mind-box">
    ...
    <div class="mind-l">
      <div class="ml-m">
        ...
        <!-- <latestChangeView /> -->
        <!-- 左侧树 -->
        <sideBarView @getNodesChange="getNodesChange" @getresult="getresult" />
      </div>
    </div>
    <!-- 右侧 -->
    <div class="mind-con">
...
export default {
  ...
  methods: {
    ...
    getlabels() {
      ...
      console.log(_this.contextRoot + "/getgraph");
      $.ajax({
        data: data,
        type: "POST",
    ...
    initgraph() {
      ...
      this.simulation = d3.forceSimulation()
        ...
        // 万有引力
        .force("charge",d3.forceManyBody().strength(-40).distanceMax(100))
        // 碰撞作用力，为节点指定一个radius区域来防止节点重叠，设置碰撞力的强度，范围[0,1], 默认为0.7。设置迭代次数，默认为1，迭代次数越多最终的布局效果越好，但是计算复杂度更高
        .force("collide", d3.forceCollide(20).strength(0.2).iterations(10))
    ...
    getresult(ms, groupno, lgraph, rgraph){
      ...
      this.initgraph();
      this.axios.get("../../../static/dataset/field"+groupno+"/"+lgraph+"vs"+rgraph+".json").then(res=> {
        // console.log(res.data);
        // var obj = JSON.parse(JSON.stringify(res.data));
        // console.log(obj);
        var ldata = res.data.lgraph;
        var rdata = res.data.rgraph;
        this.updategraph(ms, ldata, rdata);   
      });
      
    },
    updategraph(ms, ldata, rdata) {
      console.log("updategraph!");
      console.log(ldata.nodes.length);
      // console.log(ldata);
      // d3.selectAll("svg").remove(); 
      var _this = this;
      if(ms.length>0){
        this.linkGroup.exit().remove();
        this.linktextGroup.exit().remove();
        this.nodeGroup.exit().remove();
        this.nodetextGroup.exit().remove();
      }
      ...
      nodes = [];
      // for(node in nodes){
      //   _this.removeConnectedRelationships(node);
      //   _this.removeNode(node);
      // }
      for(var i = 0; i<ldata.nodes.length; i++){
        var node =new Object;
        node.name = ldata.nodes[i].id;
        // node.fx = 450- (Math.floor(Math.random() * 420));
        node.fx = (ldata.nodes[i].fx == null ? 450 - Math.floor(Math.random() * 420):parseFloat(ldata.nodes[i].fx)+50);
        node.fy = parseInt(ldata.nodes[i].fy)+80;
        node.labelName = ldata.nodes[i].labelName;
        node.uuid = parseInt(ldata.nodes[i].id);
        nodes.push(node);
      }
      var links = [];
      lks = [];
      for(var i = 0; i<ldata.edges.length; i++){
        var lk = new Object;
        lk.relation = ldata.edges[i].name;
        lk.name = "";//ldata.edges[i].name;
        lk.sourceid =parseInt(ldata.edges[i].sourceid);
        lk.targetid = parseInt(ldata.edges[i].targetid);
        // console.log(ldata.edges[i].sourceid);
        lks.push(lk);
      }

      for(var i = 0; i<rdata.nodes.length; i++){
        var node =new Object;
        node.name = rdata.nodes[i].id;
        // node.fx = 450- (Math.floor(Math.random() * 420));
        node.fx = 550 + (rdata.nodes[i].fx == null ? Math.floor(Math.random() * 420):parseFloat(rdata.nodes[i].fx));
        node.fy = parseInt(rdata.nodes[i].fy)+80;
        node.labelName = rdata.nodes[i].labelName;
        node.uuid = parseInt(rdata.nodes[i].id)+parseInt(ldata.nodes.length)+200000;
        nodes.push(node);
      }

      for(var i = 0; i<rdata.edges.length; i++){
        var lk = new Object;
        lk.relation = rdata.edges[i].name;
        lk.name = "";//rdata.edges[i].name;
        lk.sourceid = parseInt(rdata.edges[i].sourceid)+parseInt(ldata.nodes.length)+200000;
        lk.targetid = parseInt(rdata.edges[i].targetid)+parseInt(ldata.nodes.length)+200000;
        // console.log(ldata.edges[i].sourceid);
        lks.push(lk);
      }
      console.log("draw ms");
      for(var i = 0; i<ms.length; i++){
        if(ms[0]=="-1" || ms[1]=="-1")continue;
        var lk = new Object;
        lk.relation = "align";
        lk.name = "";
        lk.sourceid = parseInt(ms[i][0]);
        lk.targetid = parseInt(ms[i][1])+parseInt(ldata.nodes.length)+200000;
        // console.log(lk.sourceid, lk.targetid);
        if(ms[i][0] == ms[i][1]){
          lk.color = "green";
          // console.log(ms[i][0]);
          // console.log(ms[i][1]);
      ...
      lks.forEach(function(m) {
        ...
        links.push({ source: sourceNode.uuid, target: targetNode.uuid, lk: m ,color: m.color});
      });
      //为每一个节点定制按钮组
      // _this.addnodebutton();
      if (links.length > 0) {
        ...
        _.each(links, function(link) {
          link.maxSameHalf = Math.round(maxSame / 5);
        });
      }
      // 更新连线 links
      ...
      node.exit().remove();
      var nodeEnter = _this.drawnode(node, link, linktext);
    ...
    drawnode(node, link, linktext) {
      ...
      nodeEnter.on("mouseover", function(d, i) {
        ...
        linktext.style('fill-opacity', function(edge) {
          if (edge.source.uuid === d.uuid || edge.target.uuid === d.uuid) {
            // console.log(edge.source, edge.target, d);
            // console.log(edge.lk.relation);
            return 1;
          }
        })
        ...
        link
          .style('stroke-width', function(edge) {
            if ((typeof edge.color == "undefined" || edge.color == "") && (edge.source.uuid === d.uuid || edge.target.uuid === d.uuid)) {
              return '0.35px';
            }
          })
    ...
    drawlink(link) {
      ...
      var linkEnter = link
        ...
        //.attr("stroke", "#fce6d4")
        .style("stroke-dasharray",function(d){//dashed array for line
          // console.log(d.color);
          if (typeof d.color != "undefined" && d.color != "") {
            return "5,5";
          }
...
//   console.log('wasmWorkerInit');
//     if (!Worker || !WebAssembly) {
//         return;
//     }
//     console.log('go next!');
//     wasmWorker = new Worker("src/web/gmtWorker.js");
